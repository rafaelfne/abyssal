...
Open a pull request that closes the issue (`Closes #<issue>`), moves the Project stage to `InReview`, and reports only executed checks. Do not merge, apply approval labels or claim work is running merely because a label exists. Rafael must explicitly approve the merge.
